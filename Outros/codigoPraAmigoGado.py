...
def enviar_pedido(resposta):
    if resposta == "Sim":
        messagebox.showinfo("Pedido de Date", "Pedido aceito!")
        janela.destroy()
        ...
        def txt():
            ...
            pen.down()
            pen.color('white')
            pen.write("Aeeeeeee", font=("Verdana", 15, "bold"))
...
janela = tk.Tk()
janela.title("Pedido de Date")
janela.resizable(False, False)

# Criar canvas
canvas = tk.Canvas(janela, width=800, height=500)
canvas.pack()

# Criar degradê rosa
cor_inicio = "#FFB6C1"
cor_fim = "#850646"
canvas.create_rectangle(0, 0, 800, 500, fill=cor_inicio, width=0)
for i in range(250, 500):
    cor = '#{:02x}{:02x}{:02x}'.format(int((1 - (i - 250) / 250) * int(cor_inicio[1:3], 16) + (i - 250) / 250 * int(cor_fim[1:3], 16)),
                                       int((1 - (i - 250) / 250) * int(cor_inicio[3:5], 16) + (i - 250) / 250 * int(cor_fim[3:5], 16)),
                                       int((1 - (i - 250) / 250) * int(cor_inicio[5:7], 16) + (i - 250) / 250 * int(cor_fim[5:7], 16)),)
    canvas.create_line(0, i, 800, i, fill=cor)

# Criar rótulo
rotulo = tk.Label(janela, text="DATEZINHO, CHAMEGO E COQUINHA ZERO?", font=("Arial", 16, "bold"), fg="white", bg=cor_fim)
